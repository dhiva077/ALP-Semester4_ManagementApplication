...
export default function EditEvent() {
  ...
  const [originalEventId, setOriginalEventId] =
  ...
  useEffect(() => {
    const loadExistingEvent = async () => {
      ...
      try {
        ...
        const savedEvents = stored
          ? JSON.parse(stored)
          : {};

        const eventList =
          savedEvents[initialEventDate] || [];

        const foundEvent = eventList.find(
          (item: any) =>
            item.title === initialEventName
        );
        ...
        if (foundEvent) {
          ...
          } else if (foundEvent.fileName) {
            setSelectedFiles([
              {
                name: foundEvent.fileName,
                uri: '',
              },
            ]);
          }
          ...
          setIsLocked(
            checkIsLocked(initialEventDate)
          );
        }
      } catch (error) {
        console.error(
  ...
  const validateAndSave = async () => {
    try {
      ...
      const currentDayEvents =
        savedEvents[eventDate] || [];

      const index = currentDayEvents.findIndex(
        (item: any) =>
          item.id === originalEventId
      );

      if (index > -1) {
        currentDayEvents[index] =
          updatedEvent;
      }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        {isLocked && (
          <View style={styles.lockBanner}>
            ...
            <Text style={styles.lockText}>
              Jadwal sudah dikunci.
              Silakan lengkapi deskripsi dan
              lampiran berkas.
            </Text>
          </View>
        )}
        ...
        <Text style={styles.fileCriteria}>
          Kriteria file:
          PKS_NamaEvent,
          Invoice_NamaEvent
        </Text>
